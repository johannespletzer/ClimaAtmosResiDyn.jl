...
**The state at the end of 2026-09-18 is section 0 of
[OPERATIONAL_TODO.md](OPERATIONAL_TODO.md), and what waits for the owner is
its plan, section A.** In short:

  - #89, the merge of upstream v0.42.11, is merged (`369c8f28`).
  - #91 (C1b) and #92 (C2) have `main` merged in and are in CI. Fix what fails;
    the owner merges.
  - The runs of the day are E50 to E56 in [FINDINGS.md](FINDINGS.md).
  - `$SCRATCH/claude_work/` holds the session's working files: PR bodies
    (`pr91_body_v2.md`, `c2_pr_body_v2.md`), test environments of the old base
    (`c1b_testenv`, `c2_testenv`), and P7's measurement (`p7/`).
  - The pause of about 12:15 on 2026-09-18 ended the same afternoon. Its
    resume block is in this file's history, at `bcb02c4e`.
