...
row applies to all tags, with two denominators. Region tags use `retained / ∫tag`; source-labelled or signed tags use `retained / ∫|tag|`; below the
...
## Precondition (else the pair is inconclusive)
  ...
  - in the repair-off run, `sfc` or `rad` reaches `∫tag ≤ 0`, or `∫|tag|/∫tag ≥ 10`, at a row where `_applicable` is 1;
