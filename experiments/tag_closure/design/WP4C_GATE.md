...
**Cases.**
  ...
  - **A. D4-W, default mode**, under the follower: `configs/ wp4c_gate_d4w_default.yml`, W25's 30-level centred case
  ...
  - **B. D4-W with copies**, under the follower: `configs/ wp4c_gate_d4w_copies.yml`. Operators `vdiff` and `diffusion_up`. The
...
probe also accumulates per tag and cell `D = Σₖ (−excess − the follower's per-tag correction)`: how the tag would change if the correction took the
...
Then `python3 analysis/water/wp4c_gate_score.py $SCRATCH/tag_closure/output/wp4c_gate` reads both cases' CSVs and the
