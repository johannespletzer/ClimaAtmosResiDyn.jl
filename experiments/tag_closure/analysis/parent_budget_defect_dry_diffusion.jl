# Decision 13, review of #81: does the dry column of
...
#
#   julia +1.11 --project=.buildkite experiments/tag_closure/analysis/parent_budget_defect_dry_diffusion.jl
#
...
# `Implicit()`). Output: every defect is rounding, at most 3.8 units, for
# both diffusion models, dt 60 and 600 s, and 1 or 3 Newton iterations.
# Surface-driven diffusion at rest is identical to no diffusion.
using ClimaComms
...
for (name, vertical_diffusion) in pairs(diffusion_models), dt in (60, 600)
    for (max_iters, approximate_solve_iters) in ((1, 1), (3, 1), (1, 2), (3, 2))
        label = "$name dt=$dt newton=$max_iters approx=$approximate_solve_iters"
        try
