...
def read(d, var):
    ...
    with nc.Dataset(files[0]) as ds:
        ...
        if "lat" in ds.dimensions:
            ...
            if "z_physical" in ds.variables:
                zp = np.asarray(ds["z_physical"][:], dtype=np.float64)  # (z, lat, lon)
                zp = np.transpose(zp, (2, 1, 0))  # (lon, lat, z)
            else:
                # Without topography the levels are flat.
                z1 = np.asarray(ds[zname][:], dtype=np.float64)
                zp = np.broadcast_to(z1, (len(lon), len(lat), len(z1)))
