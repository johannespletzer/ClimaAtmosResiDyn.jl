# Decision 13, review of #81: does a dry column with a sheared wind
# (DecayingProfile plus u = 20 min(z/10 km, 1) m/s), otherwise like the one of
...
#
#   julia +1.11 --project=.buildkite experiments/tag_closure/analysis/parent_budget_defect_dry_shear.jl
#
...
# `Implicit()`). Output: every defect is rounding, at most 6.3 units, on
...
# The dry profile of `DecayingProfile`, unperturbed, with a zonal wind that
# grows from 0 at the surface to 20 m/s at 10 km.
struct ShearedProfile end
function CA.Setups.center_initial_condition(::ShearedProfile, local_geometry, params)
    state = CA.Setups.center_initial_condition(
        CA.Setups.DecayingProfile(; perturb = false, params),
        local_geometry,
        params,
    )
    (; z) = local_geometry.coordinates
    return merge(state, (; u = 20 * min(z / 10_000, one(z))))
end
