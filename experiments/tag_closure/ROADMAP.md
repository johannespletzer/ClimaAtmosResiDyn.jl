...
after the owner's review the same day):* a proposed fourth aim. Provenance
...
**What changes per milestone.**
  ...
  - **M5.** *Scope added (rev. 2):* a default is selected, for each
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD9):*
    ...
    pathway's labels are reported beside it, and that "validated" means
    Val-3: each named rule was active in the case and passed a reference that
    does not share it. Copies that are eligible validate only the rules they
    do not share.
...
OD12; revised after the owner's review):* they are reported, not scored, until
...
| Exposure screen     | The gross same-state defect of each listed rule not yet validated, times a sampled amplification `K̂` (1 until PX9 runs), plus `F`; the exact per-tag counterpart for bracketed transport such as subsidence               | Reported, never scored; a screen, not a bound; OD5's "bounded" is unchanged (OD9, OD10) |
...
| OD14 | Held-out hygiene: one held-out case, its window and its metrics, named before any rule is tuned on PX11's result; Soares, TRMM 0M and sites 23 and 26 are development cases; a held-out case never develops a rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | PX23; step 8b                                                       | PROVENANCE_PATHWAY.md, section 9                                                                                                                                                                                                                                                                                                                                                                                                     | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD14) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
not rewritten. Its reason, through G3_PLAN 6.1, is "the spread from the mixing
convention alone, about 1% in L1 (E66)". E66's L1 row is in fractions: 0.08 and
0.12 for the region tags, 0.68 to 1.26 for the source tags, so 8% to 126%.
E66's reference also keeps 5.99e5 J/m² of its own residual, and fails closure
on OD4's scale (E86). So E66's difference is not a pure convention effect, and
the reason as written does not hold. The row stands as approved, and so does
...
### Provenance pathway: exploratory numbers (2026-09-26, not thresholds)

*Scope added (provenance pathway, 2026-09-26; revised after the owner's
review):* these numbers are exploratory. They decide only what the pathway
proposes to look at next. None is a certification threshold, and none changes
an approved row.

| number                                                                    | value                                                                           | use                                                                                                                       | nearest measured (prior only)                                                                                           |
|:------------------------------------------------------------------------- |:------------------------------------------------------------------------------- |:------------------------------------------------------------------------------------------------------------------------- |:----------------------------------------------------------------------------------------------------------------------- |
| Materiality                                                               | a rule's screen above 0.2% of the tag's inventory a day, over the scored window | decides whether the next gate is proposed (PX1 to PX8, PX8 to PX16); a tenth of the per-tag L1 row, as the closure budget | W24: the repair's ledger 2.9e-3 of the column's water in a day                                                          |
...
| Reference floors                                                          | at most a quarter of the budget they judge                                      | a validity condition for a reference; proposed under OD12                                                                 | —                                                                                                                       |
...
| 6a   | Gate C: PX7, the follower's work split into lag and structure, read by monotone ratios, pre-registered before it is read (its result will be W46)                                                                                                            | step 6                                                          | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 6b   | Gate B: PX8 (the same-state subsidence probe), only if PX1 is material                                                                                                                                                                                       | step 2a                                                         | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 6c   | Gate D: PX11 (the Soares air twin, the clean label benchmark), then PX23 (one held-out case)                                                                                                                                                                 | step 6a; OD12; OD14 for PX23                                    | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 10a  | Gate E: PX22, the energy budget and `C4` checks at a fixed `c`                                                                                                                                                                                               | OD11                                                            | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
notes on existing steps, which are not rewritten.
  ...
  - **Step 8b:** it selects under rev. 2's rules as approved. If the owner
    ...
    step 8b also needs OD11, OD12 and OD14.
