...
*Scope added (provenance pathway, 2026-09-26, pending OD9 to OD14):* a proposed
fourth aim. Provenance certainty is reported per tag as a fidelity level and an
interval `[L, U]` on its error. `L` is the realized spread between admissible
rules on the same parent, and `U` bounds the propagated exposure of every rule
not yet validated. A lower rung is never evidence for a higher one, and
agreement between references that share a rule is never evidence for that rule.
The pathway, its theories and its experiments are in
[PROVENANCE_PATHWAY.md](PROVENANCE_PATHWAY.md).
...
**What changes per milestone.**
  ...
  - **M3.** *Scope added (rev. 2):* a reference that isolates tag numerics
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD10):*
    propagation (`K`) and exact per-tag counterparts, on the same state
    (PROVENANCE_PATHWAY PX8, PX9).
  ...
  - **M5.** *Scope added (rev. 2):* a default is selected, for each
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD9):*
    OD9 proposes that "bounded" means fidelity level Fid-3 with `U` within the
    provenance row, which would amend OD5. It proposes that "validated" means
    Val-3: every material rule has passed a reference that does not share it.
    Copies that are eligible validate only the rules they do not share.
...
OD12):* they are not scoring rows until the owner approves them. Section 2 of
[PROVENANCE_PATHWAY.md](PROVENANCE_PATHWAY.md) maps them onto the ladder.
...
| Propagation          | `K`, the per-tag propagator's L1 norm, from unit zero-sum spike arms; the positivity index; linearity                                                                                                                      | Reported; `K` enters `U` (OD10)                                 |
| Exposure `U`         | The gross same-state defect of each rule not yet validated, times `K`; the exact per-tag counterpart for bracketed transport such as subsidence                                                                            | `U` within the provenance row gives "bounded" (OD9, OD10)       |
| Rule sensitivity `L` | The realized same-parent difference between the admissible alternatives of each material rule, listed before the run                                                                                                       | Reported; "convention-limited" where `L` exceeds the row (OD11) |
| Reference validity   | Each reference's floors, level-1 check and contamination bound, and a certificate of which rules it does not share                                                                                                         | Floors at most a quarter of the budget (OD12)                   |

Notes on the existing rows, proposed and marked the same way:

  - **Provenance:** each result would also state its fidelity level and
    `[L, U]` per tag. An eligible comparator validates only the rules it does
    not share (OD12).
  - **Comparator eligibility:** plus an independence certificate.
  - **Convergence:** plus the follower's split into lag and structure (PX7,
    whose result will be W46).
  - **Aggregation:** proposed as a premise of "bounded" (OD9). It needs a
    band region (PP-BAND). A departure where the exchange's θ binds or the
    repair acts is expected by construction.
...
Four rules under the table, proposed and marked the same way:
  ...
  - A net first-order counterfactual sum, such as WP4c's part 3, is an
    estimate of `L`. It is never a bound or a certificate.
  - A rule's own ledgered gross bounds its error only where the faithful
    counterpart is a composition times the same gross mass. A bracket that
    sees a small net while labels move (subsidence) needs its exact
    counterpart.
  - A realized difference above its `U` voids `U` for that configuration.
  - Conventions (`c`, mask gain, donor loss, bracket granularity) are
    bracketed, never bounded, and not scored against the 2% row.
...
| OD5  | How *not assessable* is treated at M5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | M5                                                    | Proposed: it blocks M5 for that configuration unless the Insight 10 tests pass their thresholds; the configuration is then labelled "provenance bounded, not validated". *Scope added (provenance pathway, 2026-09-26): OD9 proposes to amend what "bounded" means here.*                                                                                                                        | ~~OWNER DECISION REQUIRED (OD5)~~ **Decided** 2026-09-24                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| OD7  | G4.15: bounded local movement (the same-sign rule), or the fourfold increase in gross residual it measured (E79)                                                                                                                                                                                                                                                                                                                                                                                                                                              | The G4.7 and G4.8 runs; the energy default            | Water's same-sign choice is not inherited. The owner decided on 2026-09-24 to choose after the long runs (DECISIONS.md, `design/INCREMENT_RULE_LONG_RUNS.md`). *Scope added (provenance pathway, 2026-09-26): PX3 would measure the water side. The E79 run pair would give the energy side's spread `L`. No bound exists yet, because the moved part can also change with the placement (PT6).* | ~~OWNER DECISION REQUIRED (OD7)~~ **OPEN**: deferred by the owner on 2026-09-24, until the long runs can be scored at site 23 (known issue 7)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| OD9  | The provenance ladder and its labels: the two axes; "bounded" would mean Fid-3 with `U` within the row, which amends OD5 (decided 2026-09-24), whose "bounded" rests on the Insight 10 tests alone; "validated" would mean Val-3; whether "bracketed" ever qualifies at M5; the aggregation row's form (a premise run with PP-BAND, a departure bound, or reported as now)                                                                                                                                                                                    | Any ladder label; step 8b                             | PROVENANCE_PATHWAY.md, sections 2 and 9                                                                                                                                                                                                                                                                                                                                                          | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD9) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| OD10 | The certification arithmetic: the exposure lemma (gross, not net; the exact form for bracketed transport; `K` from spike arms; the L∞ route only under positivity); completeness to rounding; the proposed numbers under "Provenance pathway proposals", after the OD3 section                                                                                                                                                                                                                                                                                | Scoring PX8, PX9 and PX16                             | PROVENANCE_PATHWAY.md, section 3                                                                                                                                                                                                                                                                                                                                                                 | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD10) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| OD11 | The rule classification in the claim contracts (G3 criterion 12, G4.3), and the admissible alternatives of each rule, including whether ψ is admissible for the leak; whether to read the 2% per-tag row as a fidelity budget (E66)                                                                                                                                                                                                                                                                                                                           | Any `L` or `U` score; PX2's rule (b), PX3, PX10, PX13 | PROVENANCE_PATHWAY.md, section 4                                                                                                                                                                                                                                                                                                                                                                 | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD11) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| OD12 | Reference validity and independence: copies certify sub-grid rules only; the air twin certifies transport rules only without sinks, subsidence or sedimentation; floors; the level-1 check; which own residual counts for TRMM 0M's copies; and a proposed reading of the Newton row: separate runs whose parents are bit for bit the same untagged twin count as same-parent, since the parent's error cancels exactly. Arms that solve their own transport (the copies, tracer mode, a passive tracer) stay gated unless the parent's error is at most 1e-3 | PX11 to PX13                                          | PROVENANCE_PATHWAY.md, sections 5 and 9                                                                                                                                                                                                                                                                                                                                                          | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD12) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| OD13 | The diagnostic-only probe PRs: PP-SUB (per-tag subsidence), PP-TRACER (a subsided passive tracer), PP-BAND (a band region) first; PP-SFC, PP-FACE, PP-JAC and PP-SRCOFF conditional; probe accounting instead of new state ledgers                                                                                                                                                                                                                                                                                                                            | Tier 2 of the pathway (step 7b)                       | PROVENANCE_PATHWAY.md, section 8                                                                                                                                                                                                                                                                                                                                                                 | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD13) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| OD14 | Held-out hygiene: the GCM site or window kept back; Soares, TRMM 0M and sites 23 and 26 are development cases; a held-out case never develops a rule                                                                                                                                                                                                                                                                                                                                                                                                          | Step 8b                                               | PROVENANCE_PATHWAY.md, section 9                                                                                                                                                                                                                                                                                                                                                                 | **PROPOSED** 2026-09-26 (provenance pathway), not yet accepted; OWNER DECISION REQUIRED (OD14) once accepted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
*Scope added (provenance pathway, 2026-09-26, pending OD11):* a note on the
approved row "Provenance, per tag at 24 h", which is not rewritten. Its reason,
through G3_PLAN 6.1, is "the spread from the mixing convention alone, about 1%
in L1 (E66)". E66's L1 row is in fractions: 0.08 and 0.12 for the region tags,
0.68 to 1.26 for the source tags. So the convention spread is 8% to 126%. The
pathway reads the 2% as a fidelity budget. It proposes that convention spreads
are reported beside the row and not scored against it. OD11 asks the owner to
confirm this reading. Until then, the row is scored as approved.
...
### Provenance pathway proposals (2026-09-26, not approved)

*Scope added (provenance pathway, 2026-09-26, pending OD10 and OD12):* none of
these is a scoring threshold until the owner approves it.

| proposal                                                                        | value                                                                                             | reason                                               | nearest measured (prior only)                                                                                           |
|:------------------------------------------------------------------------------- |:------------------------------------------------------------------------------------------------- |:---------------------------------------------------- |:----------------------------------------------------------------------------------------------------------------------- |
| Materiality                                                                     | a rule's term in `U` above 0.2% of the tag's inventory a day, over the scored window, is material | a tenth of the per-tag L1 row, as the closure budget | W24: the repair's ledger 2.9e-3 of the column's water in a day                                                          |
| Per-rule allotment, 24 h                                                        | 0.5% L1 and 1.25% L∞                                                                              | four material rules fit within the 2% and 5% row     | W40: the leak's first-order part 3, `tropo` 2.4% L1 and 6.2% L∞                                                         |
| Per-rule allotment, first hour                                                  | 0.25% L1 region, 2.5% source, 6% L∞                                                               | a quarter of the first-hour row                      | W38's P3: `evap` 11.7% between the modes                                                                                |
| Invariants and ledger completeness                                              | rounding, 1e-10 relative in Float64                                                               | exact identities                                     | W33: the clamp's cut 1.2e-15 under the follower                                                                         |
| Reference floors (OD12)                                                         | at most a quarter of the budget they judge                                                        | a reference must resolve the budget                  | —                                                                                                                       |
| The follower's part 2b per tag, D4-W (a lower bound on its worst-case exposure) | reported, not a threshold                                                                         | shows why a worst-case bound cannot pass there       | `tropo`: 3.23% (`vdiff`) and 3.97% (`sgs_mass_flux`) over 0.924 days; `strat` about 2.0% (`output/wp4c_gate/score.txt`) |
...
| 1b   | The provenance pathway's documents and PX0: archive the runs its tier 0 reads, and fault-inject its scripts                                                                                                                                                  | —                                                               | Proposed 2026-09-26 (provenance pathway); the documents are written                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 2a   | The pathway's tier 0, with no runs: PX1 (the subsidence screen) first, then PX2 to PX6                                                                                                                                                                       | step 1b; reported until OD9 to OD11                             | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 6a   | PX7: the follower's work split into lag and structure, read by monotone ratios, pre-registered before it is read (its result will be W46)                                                                                                                    | step 6                                                          | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 6b   | PX8 (the same-state subsidence probe) and PX9 (the propagation probe)                                                                                                                                                                                        | OD10                                                            | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 6c   | PX11 (the Soares air twin), PX12 (TRMM 0M comparator eligibility), PX13 (the surface-excess bracket)                                                                                                                                                         | OD11, OD12                                                      | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 7a   | PX14: WP4b's pool and the sedimentation reset, replayed in the rain-out window                                                                                                                                                                               | #121's review                                                   | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 7b   | The probe PRs and their runs: PP-SUB with PX16, PP-BAND with PX18, PP-TRACER with PX17; the conditional PRs only if triggered                                                                                                                                | OD13                                                            | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 8c   | PX21: the certification tables, the ladder level and `[L, U]` per case, tag and window; before 8b selects a default                                                                                                                                          | OD9                                                             | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 10a  | PX10: the energy repair on and off at 8 tags                                                                                                                                                                                                                 | OD11                                                            | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 10b  | PX15: the energy follower's split into lag and structure                                                                                                                                                                                                     | step 6a                                                         | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
*Scope added (provenance pathway, 2026-09-26, proposed, pending OD9 to OD14):*
notes on existing steps, which are not rewritten.

  - **Step 4:** PX5 would name the cause of the copies' repair first. Copies
    at four Newton iterations need no rerun: `w4_d4w_copies_n4` already
    failed at 0.28% a day.
  - **Step 6:** the gate's part 3 is a first-order estimate of `L`, not a
    bound. PX2 would give the realized value.
  - **Step 8b:** if the owner adopts OD9, step 8b reports the ladder level.
    It then also needs OD9, OD11, OD12, OD14 and step 8c.
  - **Step 9:** PX20 inside the one-to-two-day run; PX19 before the 90 days.
  - **Step 10:** PX10 and PX15 before G4.15 and G4.7. G4.9's placements would
    be read as `L`.
