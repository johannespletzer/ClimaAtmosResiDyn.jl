...
| sphere    | Ten days in the production configuration; *90 days since OD6 (2026-09-24)*                                                                                                                                                                                                           | V-W11                                                                                                                                | G4.13                                                                                                                                 |
...
*Scope added (provenance pathway, 2026-09-26, pending OD9 to OD14):* a fourth aim. Provenance
certainty is reported per tag as a fidelity level and an interval `[L, U]` on
its error: `L` is the realized spread between admissible rules on the same
parent, and `U` bounds the propagated exposure of every rule not yet
validated. A lower rung is never evidence for a higher one, and agreement
between references that share a rule is never evidence for that rule. The
pathway, its theories and its experiments are in
...
**What changes per milestone.**
  ...
  - **M2.** *Scope added (rev. 2):* the single verdict "independent
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD10):* the Invariants and
    Ledger-completeness rows proposed below the contract.
  - **M3.** *Scope added (rev. 2):* a reference that isolates tag numerics
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD10):* propagation (`K`) and
  ...
  - **M5.** *Scope added (rev. 2):* a default is selected, for each
    ...
    *Scope added (provenance pathway, 2026-09-26, pending OD9):* OD9 proposes that
    "bounded" means fidelity level Fid-3 with `U` within the provenance row,
    and that "validated" means Val-3: every material rule has passed a
    reference that does not share it. Copies that are eligible validate only
    the rules they do not share.
...
**Proposed rows** (Scope added (provenance pathway, 2026-09-26, pending OD9 to OD12). They
are not scoring rows until the owner approves them. Section 2 of
...
Notes on the existing rows, marked the same way:

  - **Provenance:** each result also states its fidelity level and `[L, U]`
    per tag. An eligible comparator validates only the rules it does not
    share (OD12).
  - **Comparator eligibility:** plus an independence certificate.
  - **Convergence:** plus the follower's split into lag and structure, by
    the method of W46 (PX7).
  - **Aggregation:** proposed as a premise of "bounded" (OD9). It needs a
    band region (PR-P4). A departure where the exchange's θ binds or the
    repair acts is expected by construction.

Four rules under the table, marked the same way:
  ...
  - Conventions (`c`, mask gain, donor loss, bracket granularity) are
    bracketed, never bounded, and never scored against the 2% row.
...
*Scope added (provenance pathway, 2026-09-26):* OD9 to OD14 at the end of the table
are proposed by [PROVENANCE_PATHWAY.md](PROVENANCE_PATHWAY.md). They are open
once the owner enters them. Until then OD7 is still the only open numbered
decision.
...
| OD5  | How *not assessable* is treated at M5                                                                                                                                                                                                                                                                                                                                                                   | M5                                                    | Proposed: it blocks M5 for that configuration unless the Insight 10 tests pass their thresholds; the configuration is then labelled "provenance bounded, not validated". *See OD9 (proposed 2026-09-26).*                                                     | ~~OWNER DECISION REQUIRED (OD5)~~ **Decided** 2026-09-24                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| OD9  | The provenance ladder and its labels: the two axes; "bounded" means Fid-3 with `U` within the row; "validated" means Val-3; whether "bracketed" ever qualifies at M5; the aggregation row's form (a premise run with PR-P4, a departure bound, or reported as now)                                                                                                                                      | Any ladder label; step 8b                             | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, sections 2 and 9)                                                                                                                                                                                                 | OWNER DECISION REQUIRED (OD9), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| OD10 | The certification arithmetic: the exposure lemma (gross, not net; the exact form for bracketed transport; `K` from spike arms; the L∞ route only under positivity); completeness to rounding; the proposed numbers in the block after the OD3 table                                                                                                                                                     | Scoring PX8, PX9 and PX16                             | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, section 3)                                                                                                                                                                                                        | OWNER DECISION REQUIRED (OD10), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OD11 | The rule classification in the claim contracts (G3 criterion 12, G4.3), and the admissible alternatives of each rule, including whether ψ is admissible for the leak; confirmation that the 2% per-tag row is a fidelity budget (E66)                                                                                                                                                                   | Any `L` or `U` score; PX2's rule (b), PX3, PX10, PX13 | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, section 4)                                                                                                                                                                                                        | OWNER DECISION REQUIRED (OD11), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OD12 | Reference validity and independence: copies certify sub-grid rules only; the air twin certifies transport rules only without sinks, subsidence or sedimentation; floors; the level-1 check; which own residual counts for TRMM 0M's copies; and a proposed amendment to the Newton row for swaps of follower rules between runs whose parents are bit for bit the same untagged twin, if PX2 (a) passes | PX11 to PX13                                          | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, sections 5 and 9)                                                                                                                                                                                                 | OWNER DECISION REQUIRED (OD12), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OD13 | The diagnostic-only probe PRs: PR-A (per-tag subsidence), PR-S (a subsided passive tracer), PR-P4 (a band region) first; PR-B, PR-D, PR-E and PR-P2 conditional; probe accounting instead of new state ledgers                                                                                                                                                                                          | Tier 2 of the pathway (step 7b)                       | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, section 8)                                                                                                                                                                                                        | OWNER DECISION REQUIRED (OD13), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OD14 | Held-out hygiene: the GCM site or window kept back; Soares, TRMM 0M and sites 23 and 26 are development cases; a held-out case never develops a rule                                                                                                                                                                                                                                                    | Step 8b                                               | Proposed 2026-09-26 (PROVENANCE_PATHWAY.md, section 9)                                                                                                                                                                                                        | OWNER DECISION REQUIRED (OD14), proposed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
**Provenance pathway proposals** (Scope added (provenance pathway, 2026-09-26, pending OD9
to OD12). These are not scoring thresholds until the owner approves them.

| proposal                                  | value                                                                         | reason                                               | nearest measured (prior only)                                                                       |
|:----------------------------------------- |:----------------------------------------------------------------------------- |:---------------------------------------------------- |:--------------------------------------------------------------------------------------------------- |
| Materiality                               | a rule's term in `U` above 0.2% of the tag's inventory per window is material | a tenth of the per-tag L1 row, as the closure budget | W24: the repair's ledger 2.9e-3 of the column's water in a day                                      |
...
| Reference floors                          | at most a quarter of the budget they judge                                    | a reference must resolve the budget                  | —                                                                                                   |
| Worst-case exposure of the follower, D4-W | reported, not a threshold                                                     | shows why a worst-case bound cannot pass there       | `tropo`: 3.23% (`vdiff`) and 3.97% (`sgs_mass_flux`) over 0.924 days (`output/wp4c_gate/score.txt`) |

*Scope added (provenance pathway, 2026-09-26, pending OD11):* a note on the approved
row "Provenance, per tag at 24 h", which is not rewritten. Its reason,
through G3_PLAN 6.1, is "the spread from the mixing convention alone, about
1% in L1 (E66)". E66's L1 row is in fractions: 0.08 and 0.12 for the region
tags, 0.68 to 1.26 for the source tags. So the convention spread is 8% to
126%, and the 2% is a fidelity budget. Convention spreads are reported beside
it and never scored against it. The owner confirms this reading in OD11.
...
| 6a   | PX7: W46 read by monotone ratios, lag or structure, pre-registered before it is read                                                                                                                                                                         | step 6                                                          | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 7b   | The probe PRs and their runs: PR-A with PX16, PR-P4 with PX18, PR-S with PX17; the conditional PRs only if triggered                                                                                                                                         | OD13                                                            | Proposed 2026-09-26 (provenance pathway)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
*Scope added (provenance pathway, 2026-09-26):* notes on existing steps, which are
not rewritten.

  - **Step 4:** PX5 names the cause of the copies' repair first. Copies at
    four Newton iterations are not rerun: `w4_d4w_copies_n4` already failed
    at 0.28% a day.
  - **Step 6:** the gate's part 3 is a first-order estimate of `L`, not a
    bound. PX2 gives the realized value.
  - **Step 8b:** it reports the ladder level, and needs OD9, OD11, OD12,
    OD14 and step 8c.
  - **Step 9:** PX20 inside the one-to-two-day run; PX19 before the 90 days.
  - **Step 10:** PX10 and PX15 before G4.15 and G4.7. G4.9's placements are
    read as `L`.
