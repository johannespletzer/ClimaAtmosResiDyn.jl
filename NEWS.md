...
- ![][badge-🔥behavioralΔ] The tag families' closure checks no longer end a run by default. `water_closure_check`'s `abort_above` defaulted to 1.0, and that ended runs whose parent's own water had gone negative while the model ran on: a diagnostic ended a run that upstream completes. `abort_above` now defaults to `~` for every family. A new key, `void_above`, takes water's old level, 1.0: above it the check warns once and writes a last column `void`, 1 on this and every later row, in the closure and audit tables, and the run goes on. The energy families default to `void_above: ~`. An explicit `abort_above` still ends the run. The model's fields are unchanged: the check only reads the state and writes its tables.
...
- ![][badge-🔥behavioralΔ] `energy_source_tag_transport: enthalpy_increment` is refused with microphysics that sediments (`microphysics_model` 1M, 2M or 2MP3) stepped explicitly (`implicit_microphysics: false`), unless the new key `energy_source_tag_increment_allow_explicit_microphysics: true` is set. The parent's `ρe_tot` row in the implicit Jacobian has a cross block from each sedimenting species and the tags' rows do not, so with one Newton iteration the tags lag the parent's sedimentation, and the lag lands in `e_src_res`: with 1M on a DYCOMS RF02 EDMF column 2.1e-4 of the partitioned energy after an hour, against 1.5e-6 with the microphysics implicit. 2M and P3 are not measured, and are refused until they are. PR #105 proposes the analogous cross blocks for the water tags under 1M, and PR #113 the energy tags' own; until those land, neither family's tags carry them here. The key is an override for development runs, off by default, and the model warns when it is used. The refusal concerns only the tags' keys, so a configuration without the tags, with another transport, or with 0M, runs as before.
