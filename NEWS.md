...
main
----
- [#49](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/49) ![][badge-✨feature/enhancement] Added the parent-budget ledger's journal core in the new `src/parent_budget/` subtree: the leg schema, the authoritative endpoint integrals and the per-step transaction. Internal only — nothing is wired into the timestepper, no tendency is instrumented, and no simulation output changes.
