...
- [#48](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/48) ![][badge-✨feature/enhancement] Added the parent-budget ledger's plan, budget contract and coverage matrices as three Developer Guide pages under `docs/src/parent_budget/`. Design only: no instrumentation, no code, and no change to any simulation. The pages freeze what an exact signed discrete mass, total-water and total-energy ledger will claim, and inventory every equation and state-mutation path against that claim.
