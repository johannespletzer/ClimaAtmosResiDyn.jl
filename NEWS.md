...
main
----
- ![][badge-🐛bugfix] Put `ChangesOfVariables` in the test environment, so Aqua's persistent-task check can walk the dependency tree. Aqua 0.8.17 walks it with `Base.locate_package` for every name in each package's `[deps]` section and does not skip the names that also stand in `[weakdeps]`. `LogExpFunctions` v1.0.1 lists three names twice, and one of them, `ChangesOfVariables`, is in nobody else's manifest, so both `infrastructure` jobs failed with "Unable to locate `ChangesOfVariables`". Aqua 0.8.16 resolved the environment through Pkg and did not. The package is a test-only dependency now; nothing loads it. Remove it once Aqua skips `[weakdeps]` there.
