...
main
----
- ![][badge-💥breaking] Under `energy_source_tag_transport: enthalpy_increment`, two columns of the energy source tags' audit table are renamed: `increment_left_gross` is now `increment_left_net_abs`, and `increment_moved_gross` is now `increment_moved_net_abs`. Their `_relative` columns follow, as `increment_left_net_abs_relative` and `increment_moved_net_abs_relative`. The values keep their definition. Each is the sum over the cells of the absolute value of that cell's ledger, and the ledger is net over time in each cell. So they are not a throughput, which the old names suggested. The water tags' follower uses the same names. A script that reads these columns by name needs the new names. The same mode now needs the region tags' masks to sum to 1 within floating-point tolerance, 100 rounding units of the float type (2.2e-14 in Float64, 1.2e-5 in Float32), as the water tags' follower does. Before, a sum within 1% of 1 passed. A configuration whose masks leave a gap or an overlap below 1% now stops when the cache is built, with an error that names the deviation. Every process takes the largest deviation over all processes, so they all stop together.
