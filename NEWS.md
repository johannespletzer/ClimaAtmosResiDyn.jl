...
main
----
- ![][badge-✨feature/enhancement] Add online closure checking for the tagged tracers, under two new config keys `water_closure_check` and `energy_closure_check`. Each periodically reduces its family's closure to one number — `relative = (total - Σ pure region tags) / total`, from the volume-weighted global integrals — appends it to `water_tag_closure.csv` / `energy_tag_closure.csv`, and warns when it exceeds a tolerance. Previously closure was only visible after the fact, by outputting the `q_tag_res` / `e_tag_res` fields and post-processing them.
  ...
  - The check adds no tendency — it reads the state and writes a table — so simulation output is unchanged. `baroclinic_wave_tagged_water.yml` and `baroclinic_wave_tagged_tracers.yml` switch it on so CI exercises it.
...
- ![][badge-✨feature/enhancement]![][badge-💥breaking] Give the three tracer families one configuration interface, under three sibling top-level keys: `passive_tracers`, `water_tracers` and `energy_tracers`. They replace 16 keys that were spread across two parts of the schema under three unrelated shapes, and the schema is 13 keys smaller. Simulation output is unchanged: the models built from the migrated configs are identical to those built from the old keys.
  ...
  - Retired keys raise an error naming their replacement, listing every offending key at once, so a config migrates in one pass. Previously an unknown key was only a warning, which for something like `tracer_production_rate` would have changed the science of a run without saying so.
  ...
  - New user-facing page `docs/src/tracer_configuration.md` is the configuration reference for all three families, with a chooser table, copy-paste starter blocks and an old → new migration table. The three existing pages keep the physics and link to it. A new test asserts that every shipped config file only sets keys the schema defines, which nothing checked before.
