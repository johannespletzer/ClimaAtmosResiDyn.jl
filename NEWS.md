...
main
----
- ![][badge-✨feature/enhancement] The energy source tags now carry sedimentation cross blocks in the manual Jacobian, as the water tags do. Each tag's row has its face share of the block of `ρe_tot + c·ρ` to every falling species, solved after the model's fields by the split solver, so the model's increments do not change. With one Newton iteration and 1-moment microphysics stepped explicitly, the tags lagged the parent's implicit sedimentation without them (2.1e-4 of the energy in an hour on a DYCOMS RF02 EDMF column under `enthalpy_increment`). Without the split solver (`use_auto_jacobian: true`) there are no cross blocks. See docs/src/energy_source_tags.md.
