...
main
----
- ![][badge-✨feature/enhancement] The energy source tags' updraft copies (`energy_source_tag_updraft_copy: true`) now mirror the four things the model gives the updraft's `mseʲ` and no updraft tracer: the surface enthalpy flux into the updraft's lowest cell, the relaxation there toward the surface's buoyant air, radiation under RRTMGP, and the updraft's 0-moment rain-out. Each goes to the copies by the grid mean's rule for its label, so the partition's copies change as `mseʲ` does. A new diagnostic, `e_src_copy_res`, shows what the mirrors do not cover. The model's fields do not change. See docs/src/energy_source_tags.md.
