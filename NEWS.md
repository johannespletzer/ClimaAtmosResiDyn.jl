...
main
----
- ![][badge-🔥behavioralΔ] `energy_source_tag_transport: enthalpy_increment` is refused with 1-moment microphysics stepped explicitly (`implicit_microphysics: false`), unless the new key `energy_source_tag_increment_allow_explicit_1m: true` is set. The parent's `ρe_tot` row in the implicit Jacobian has a cross block from each sedimenting species and the tags' rows do not, so with one Newton iteration the tags lag the parent's sedimentation, and the lag lands in `e_src_res`: on a DYCOMS RF02 EDMF column 2.1e-4 of the partitioned energy after an hour, against 1.5e-6 with the microphysics implicit. The water tags now carry those blocks; the energy tags do not yet. The key is an override for development runs, off by default, and the model warns when it is used. The refusal concerns only the tags' keys, so a configuration without the tags, or with another transport, runs as before.
