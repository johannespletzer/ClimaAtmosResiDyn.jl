...
main
----
- ![][badge-✨feature/enhancement] Under `energy_source_tag_transport: enthalpy` with `turbconv: prognostic_edmfx` or `edonly_edmfx`, the energy source tags now take their shares of the parent's own vertical EDMF eddy diffusion. The parent diffuses `ρe_tot` in enthalpy form, `-ρK_h ∇s_d - ρK_e ∇h_tot - ρK_h (h_eff + Φ) ∇q_tot_eff`, and `ρ` with its water, and the flux of `ρe_tot + c·ρ` is shared out by the cell it leaves, as the sub-grid mass flux is. The tags leave the tracer diffusion loop there and lose their diffusion blocks in the implicit Jacobian. The flux runs beside the parent's, without a Jacobian block of its own, so within a step the tags lag the parent's implicit diffusion, and that gap lands in `e_src_res`. The default `tracer` transport is unchanged, and so is the model.
