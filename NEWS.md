...
main
----
- ![][badge-✨feature/enhancement] The water closure check reads the parent's own negative water (known issue 7). Under option C the tags partition `max(ρq_tot, 0)`, so the closure can pass while the parent is negative. Every row of `water_tag_closure.csv` now ends with `negative_water_relative`, `∫max(-ρq_tot, 0) dV / ∫ρq_tot dV` from the raw `ρq_tot`, and `negative_water_void`, which latches at 1 once that passes the new key `negative_water_void_above` (default `1e-4`, the tag-closure contract's level; `~` drops both columns). The checkpoint carries the flag, as it carries `closure_void`. A ledger in the cache adds `max(-ρq_tot, 0) Δt` after every accepted step and counts the negative cells, so that the audit table, `water_tag_audit.csv`, reports the negative water between two checks: `negative_water_integral` (kg s), `negative_water_interval`, `negative_water_interval_mean_relative` and `negative_water_interval_events`, which is 0 exactly when no step in the interval had negative water. The checkpoint carries the ledger too, and a checkpoint written before it restarts it at zero with a warning. New opt-in diagnostics: `q_tag_negative_integral` (kg s m⁻³) and `q_tag_negative_events`. The audit's `nonpositive_mass` for water reads the raw `ρq_tot` again: under option C it read the target and was 0 by construction. The model's fields are unchanged.
- ![][badge-🐛bugfix] Water tags no longer diverge where the model's own water goes negative (known issue 7). The partition tags partition the non-negative part, `max(ρq_tot, 0)`, and a new diagnostic, `q_tag_negative`, reports the negative part. Under `water_tag_transport: increment` the follower takes the non-negative part's increment: a cell a solve takes below zero gives up its tags by the partition's own composition, and the water the parent creates elsewhere goes to the tags there, in a new ledger, `q_tag_inc_negative`. The limiters' rescale, the copies' repair, `q_tag_res` and the closure check use the non-negative part. The tag name `negative` is reserved. Where the parent is never negative, the tags are unchanged bit for bit; the model's fields never change. See docs/known_issues.md, issue 7.
- ![][badge-🔥behavioralΔ] The tag families' closure checks no longer end a run by default. `water_closure_check`'s `abort_above` defaulted to 1.0, and that ended runs whose parent's own water had gone negative while the model ran on: a diagnostic ended a run that upstream completes. `abort_above` now defaults to `~` for every family. A new key, `void_above`, takes water's old level, 1.0: above it the check warns once and writes a last column `closure_void`, 1 on this and every later row, in the closure and audit tables, and the run goes on. The checkpoint records the flag, so the rows after a restart stay marked; a checkpoint written before restarts as not void, with a warning. `closure_void` is about the closure only. It is 0 until the residual passes the level, and says nothing about the parent: at site 23 of the tag-closure long runs the parent's water went negative from day 10, and the closure passed 1.0 only at day 48 or day 74.5, depending on the run. The existing column `nonpositive_fraction` reports where the parent is not positive. The energy families default to `void_above: ~`. An explicit `abort_above` still ends the run. The model's fields are unchanged: the check only reads the state and writes its tables.
- [#109](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/109) ![][badge-✨feature/enhancement] Each tag's own ledgers, and what the steps retained beside what the writers attempted. Two opt-in keys, both off by default: `water_tag_ledger_per_tag: true` gives each water tag the state fields `q_tag_led_fix_<name>`, what the limiters' rescale and the partition repair changed it by, and under `water_tag_transport: increment` `q_tag_led_inc_<name>`, what the follower moved into or out of it. `energy_source_tag_ledger_per_tag: true` gives each energy source tag `e_src_led_fix_<name>`, for the repair, and under `enthalpy_increment` `e_src_led_inc_<name>`. A ledger's grosses are the diagnostics `<L>_gross` and `<L>_colgross`; for a tag's own ledger the kind goes before the tag's name, as in `q_tag_led_fixgross_<name>`, so that no tag's name can collide. The closure audit (`audit: true`) reports per state ledger `L`: `<L>_retained`, what the accepted steps kept, `Σ|ΔL|` over the steps, integrated; `<L>_attempted`, the absolute value of what its writers added over every call, stage values the stepper discards included; and `<L>_events`, the cell-steps whose change exceeded rounding. Each is also over the closure's scale, `_relative`. The cadence limits them: a tag's own ledgers are exact at every `update_constrain_state_every`, the ledgers per mechanism only at `step`, and at `stage` or `dss` the model warns once and `ledger_cadence_step` is 0. For each tag's own ledgers the audit also gives three ratios of `_retained` and a flag. `_inventory_fraction` divides by `∫tag`, and serves a pure region tag, which needs a positive inventory. `_burden_fraction` divides by `∫|tag|`, and serves a source tag or any tag with negative parts, where the signed total can be zero or nearly cancel. `_parent_fraction` divides by the parent scale, `∫ρq_tot` for water and, for energy until OD4's source throughput is in the model, the process records' `Σₚ∫|prc_e_p|`, `NaN` without records. `_applicable` is 0 where the tag's burden is zero or below 2e-4 of the parent scale; then no ratio to the tag is read, and the tag is judged by `_parent_fraction`. See `docs/src/tagged_water.md` and `docs/src/energy_source_tags.md`. The model's fields do not change.
- [#109](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/109) ![][badge-🔥behavioralΔ] The checkpoint carries the tags' cache accumulators, and a restarted run continues them: the repair ledgers `q_tag_fix_<name>`, `q_tag_upfix_<name>` and `e_src_fix_<name>` with their gross twins and counts, and each state ledger's per-step gross, column gross, events and attempted total. They no longer start again at zero at a restart. The state ledgers continue as fields of the state, as before. A checkpoint written before this holds none of the accumulators. It restarts with them at zero and a warning, and begins a new accumulator segment: their totals, the audit's among them, cover that segment only and are not whole-run totals. A checkpoint that holds only some of them is refused.
- ![][badge-✨feature/enhancement] `water_tracers` run under `turbconv: prognostic_edmfx` with one updraft. By default each tag takes its share of the parent's sub-grid mass flux of water, from the donor cell, and an exchange of provenance at the updraft's mass flux, which sums to zero over the partition, moves the updraft's composition from a steady entraining plume rescaled to the updraft's water. `water_tag_updraft_copy: true` instead gives each tag a copy in the updraft, `q_tag_<name>`, which the model moves as any other updraft tracer, with the updraft's 0M rain-out, 1M sedimentation, surface relaxation, surface moisture flux and repair after the filter mirrored on it. More than one updraft stays refused, and the copies are refused without prognostic EDMF or with unequal updraft upwinding. New diagnostics: `q_tag_upfix_<name>` and `q_tag_copy_res` for the copies, and `q_tag_leak_<path>`, the rate at which each path that diffuses the tags on their whole value, and `ρq_tot` without rain and snow, drifts the partition. The water closure audit reports where the exchange's bound binds. A restart that changes a water tag's region or sources, or adds or drops the copies, is refused. `docs/known_issues.md` restates issue 3.
- ![][badge-💥breaking] `water_tracers` are refused under `turbconv: prognostic_edmfx` and under `amd_les: true`, which were accepted silently before. Under prognostic EDMF the tags have no updraft fields, so they miss the updraft's mass flux of water, and the partition drifts from `ρq_tot`. The refusal lasts until the tags follow the updrafts. AMD diffuses each tracer with a diffusivity from its own gradient, so in general the tags' diffusion does not add up to that of `ρq_tot`. `water_process_record` stays allowed under both, since its records are not transported. A run with water tags and a prescribed flow, from its setup or the `prescribed_flow` key, now warns that the flow's surface moisture flux enters `ρq_tot` untagged. Tag names starting with `fix_`, `upfix_`, `inc_`, `rtag_` or `stag_` are reserved in `water_tracers`, because they can take the name of another diagnostic of the family, now or once it exists. `docs/known_issues.md` closes issue 1 with the numbers after #64's fix, restates issue 3, and adds to issue 4 a first measurement on a raining 0M column, which bounds the region tags' Newton sensitivity but does not isolate the missing Jacobian diagonal; issue 4 stays open.
- ![][badge-🐛bugfix] A run that takes its initial state from a file no longer starts every region tag as `NaN`. `WeatherModel`, `AMIPFromERA5` and `MoistFromFile` build the pointwise state from `NaN` placeholders and then rewrite `ρ`, `ρe_tot`, `ρq_tot`, the condensates and the winds from the file. The tags were built from the placeholders and nothing built them again, so every region tag of `ρe_tag_*`, `ρe_src_*` and `ρq_tag_*` held `NaN` and the run stopped at the first check, where the same configuration without tags ran. `initial_state` now calls `rebuild_tags_from_state!` after the setup's overwrite, which sets each tag by the rule that built it: a pure region tag takes its masked share of the parent, every other tag zero, and the energy source tags' updraft copies follow their tags. It is idempotent where nothing overwrote the state, and a restart does not go through it, so a checkpoint's tags are untouched.
- ![][badge-✨feature/enhancement] The energy source tags' closure check now has a default `tolerance`, one per `energy_source_tag_transport`: 1.0 under `tracer`, 0.1 under `enthalpy` and 0.01 under `enthalpy_increment`, in `ENERGY_SOURCE_CLOSURE_TOLERANCES`. They are calibrated from the 59 runs of the tag-closure experiments, about an order of magnitude above the largest value each transport showed in a healthy run, so they warn when the tags hold energy far from what the parent has rather than when a configuration merely differs. Set `tolerance` in the block for a tighter level, or `~` to warn about nothing, as before. The new page `docs/src/energy_source_tags_guide.md` is a user guide for running the tags and reading their output, with the caveats.
- ![][badge-✨feature/enhancement] The energy source tags now mix provenance through the updrafts. Under `turbconv: prognostic_edmfx` each tag took its share of the parent's sub-grid mass flux of `ρe_tot + c·ρ` from the cell the flux leaves, which moves the net energy convection carries but not the exchange of air behind it. Each tag now also takes an exchange at the mass flux, `Σₖ ρᵏaᵏ(u³ᵏ - u³)(φᵏ - φ̄)(e_totᵏ + c)`, with the updraft's shares from a steady entraining plume built from the model's own entrainment. It sums to zero over the partition, so closure is untouched. It needs region tags without sources that partition the domain, and a configuration without them is refused at initialization, unless the tags have updraft copies. The new key `energy_source_tag_updraft_copy`, `false` by default, switches to an audit: each tag gets a passive copy in the updraft, `e_src_<name>`, which the model moves as any other updraft tracer. It needs `enthalpy_increment` for closure, or `tracer`, and is refused with `enthalpy` and without `prognostic_edmfx`. A restart refuses a change of the switch. The model's fields are unchanged, bit for bit, in both modes.
- ![][badge-✨feature/enhancement] A prototype, `energy_source_tag_transport: enthalpy_increment`: the enthalpy audit with the energy source tags following the parent's own implicit increment. After each Newton solve, the mismatch between the parent's increment of `ρe_tot + c·ρ` and the tags' is moved as a vertical flux and shared out by the cell it leaves; the part that changes a column's total stays in `e_src_res`. Tags that follow a stiff implicit term by its tendency drift from the parent step by step, and this removes that gap. It needs an offset, region tags that partition the domain, an algorithm that solves every stage whose implicit tendency it uses (the default ARS343 does; SSP333 and the IMKG algorithms do not), and an `energy_q_tot_upwinding` other than `none`, and it refuses a configuration without them. The model is untouched. A ledger of two prognostic fields, `e_src_inc_left` and `e_src_inc_moved`, keeps what the correction left out of the tags and what it moved between levels, and the closure check's audit table reports their integrals. The new test group `tagging_source_increment` checks the correction, and that the model's fields on the EDMF column are bit for bit those without tags.
- ![][badge-🐛bugfix] The process records are no longer advected horizontally. The horizontal advection of tracers and the SEM limiter take every state field `is_tracer_var` accepts, not only the `ρ`-prefixed tracers, so on a sphere they moved and limited the `prc_e_*` and `prc_q_*` records as if the air carried them. A record is the history of what one process did in a cell, and `docs/src/process_record.md` says nothing moves it. `is_tracer_var` now asks for the `ρ` prefix, as `gs_tracer_names` does. Every tracer the model carries has it, and the records do not. Column runs, which have no horizontal advection, are unchanged, and so is every other field on a sphere: the records feed back into nothing.
- ![][badge-✨feature/enhancement] A restart can no longer change what the energy source tags in a checkpoint mean. A checkpoint now records `energy_source_tag_offset`, each tag's region and sources, `energy_source_tag_transport` and `energy_source_tag_repair`, and a restart that changes one stops before the cache is built, with an error that names the setting and both values. The tags' and the process records' fields must also match the configuration. A checkpoint written before this is checked by its fields alone, with a warning. There is no override: to change a setting, start a new run. `tag_region_spec` and `tag_region_text` give a region in the words of its configuration.
- ![][badge-✨feature/enhancement] The energy source tags now run under `turbconv: prognostic_edmfx` with one updraft. The tags have no updraft copy. So each takes its share of the parent's own sub-grid mass flux of `ρe_tot + c·ρ`, face by face, from the cell the flux leaves. Under 1-moment microphysics each also takes its share of the updraft and environment corrections to sedimentation, shared once with the grid mean's flux by the direction of their sum. The partition's fluxes add up to the parent's in every evaluation of the tendency. The tags have no Jacobian block for either, so within a step they lag the parent's implicit flux slightly, and that gap lands in `e_src_res`. More than one updraft is refused at configuration time. The model runs `prognostic_edmfx` with one updraft only, and computes the sedimentation corrections for the first updraft only. The eddy diffusion still moves the tags as passive tracers, and the model warns. The updrafts' vertical diffusion (`edmfx_vertical_diffusion: true`) now skips a grid-scale tracer the updraft does not carry, as the horizontal path already did, so a grid-scale-only tracer no longer makes it fail. Every field the updraft carries gets the same tendency as before. The new test group `tagging_source_edmf` checks all of it on the DYCOMS RF02 EDMF column.
...
- ![][badge-✨feature/enhancement] Add `energy_source_tag_repair`, on by default. Where the total they partition is positive, it keeps the energy source tags non-negative after each state update. The pure region tags keep their sum and give up a negative tag's deficit in proportion to what each holds, as the water tags' partition repair does, and a tag that carries a source is clipped at zero. Every change is logged in the new diagnostic `e_src_fix_<name>`. The repair does not force the tags onto the total, so `e_src_res` still shows the transport mismatch. `false` leaves the tags as the attribution rule and their transport make them, negative values included.
- ![][badge-✨feature/enhancement] Add `energy_source_tag_offset`, an energy per kilogram of air that the energy source tags add to `ρe_tot` to form the total they partition, `ρe_tot + c·ρ`. The model never uses that total, so the simulated atmosphere is unchanged, while a large enough offset makes the total positive where `ρe_tot` is not and lets the donor rule run there. Moving the thermodynamic reference instead gives the tags the same total in exact arithmetic, but it reaches the model's numerics. An offset of `0` leaves the tags on `ρe_tot` exactly as before. See docs/src/energy_source_tags.md.
- [#64](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/issues/64) ![][badge-🐛bugfix] Stop the water tags diverging while `ρq_tot` stays bounded. `rescale_water_tags!` followed a limiter or state constraint by multiplying every tag by `r = ρq_tot_after / ρq_tot_before`. Writing `e = ρq_tot - Σₖ ρq_tagₖ` for the closure error the `q_tag_res` diagnostic reports, that gave `e_after = r · e_before` in every cell with a positive parent — a no-op on the error only when the error was already zero. The SEM quasimonotone limiter's lower bound is the element minimum of `q` from the reference state, so a node sitting below it is lifted every stage with `r > 1`; on a sphere that compounded at about 1.4 per limiter application and reached a relative residual of 6e113 in 24 simulated hours, with tags at 1e130 against a `ρq_tot` that stayed inside 1.6185e16 to 1.6214e16. The run reported success.
  - The correction is now additive. The parent's increment `Δ = ρq_tot_after - ρq_tot_before` is handed to the tags under the donor rule the rest of the file already uses, `ρq_tagₖ += Δ · sₖ`, with `sₖ` renormalized over the partition so the shares sum to one. On a partition that is closed and non-negative this is the *same expression* as the old factor, so nothing changes for a well-behaved cell; the two differ only on `e`, which is exactly where multiplying was wrong. The new recursion is `e_after = e_before` wherever the partition holds water and the loss floor does not bind, and `e_after = e_before + min(Δ + Σₖ max(ρq_tagₖ, 0), 0)` where it does — bounded by `|Δ|`, never a multiple of `e_before`.
  - Non-negativity is preserved rather than restored, by the same mechanism as before: the loss the tags absorb is floored at what they hold, which on a closed partition reproduces the old ratio's floor at zero, so no non-negative tag is driven below zero. A tag that is already negative is not lifted by this correction — its share is zero — and waits for `repair_water_tag_partition!`, which is the correction that exists for it. A source tag keeps its own unnormalized donor share, floored at the parent, mirroring the split `sediment_water_tags!` already makes. The `ρq_tot_before ≤ 0` branch still empties every tag, for the reason its docstring gave. All of it is still recorded in `q_tag_fix_<name>` before it is applied.
  - The repair still does **not** renormalize the tags onto `ρq_tot`. Driving `q_tag_res` to zero by construction would destroy the leakage monitor, and this fix bounds the residual rather than removing it. `water_tag_rescale_ratio` is replaced by `water_tag_rescale_shift` and `water_tag_source_rescale_shift`; the cache fields `ᶜrepair_pos` / `ᶜrepair_neg` are now `ᶜwater_pos` / `ᶜwater_neg`, since both corrections use them.
- ![][badge-✨feature/enhancement] Give the tag closure check a level at which it ends the run, as `abort_above` in the `water_closure_check`, `energy_closure_check` and `energy_source_closure_check` blocks. Exceeding `tolerance` still warns and keeps running, which is right for drift and wrong for a relative residual of 1e113. **Water defaults to `1.0`, so an existing water-tagged run with the check enabled can now stop where it used to warn.** A set of non-negative tags inside a non-negative parent misses it by at most the parent itself, pointwise, so an honest partition cannot reach 1 and neither can an honest strict subset of one; passing 1 means the tags hold water that is not there. Both energy families default to `~` (no level), because their residual is normalized by `∫|ρe_tot|`, whose zero is a convention — a shifted reference makes the ratio arbitrarily large with nothing wrong. Write `abort_above: ~` to turn the water default off. The abort is raised on every process, not on the root alone, so it cannot leave the others waiting in the next reduction.
- ![][badge-✨feature/enhancement] Add an audit mode to the tag closure check, as `audit` in the `water_closure_check`, `energy_closure_check` and `energy_source_closure_check` blocks. Off by default; it costs a handful of extra global reductions per check and changes nothing about the run. It writes `<family>_tag_audit.csv` beside the closure table, a separate file so that the closure table's schema stays as it is for whatever already reads it. `gross_relative` is the right number to test a tolerance against and the wrong number to diagnose with, because it adds together situations that are not the same problem. The audit separates them: `untagged` (`∫max(parent - Σ tags, 0)`, water with no origin that nothing claims to know) and `overclaimed` (`∫max(Σ tags - parent, 0)`, tags asserting water that does not exist, which is the direction a runaway takes) sum to exactly `gross_residual`; `orphaned` is the mass in cells whose parent still holds water while every tag is empty, which is provenance destroyed rather than drifting, since nothing re-tags a cell afterwards; and `nonpositive_mass` is the mass counterpart of the closure table's volume `nonpositive_fraction`. Those last two answer different questions and can disagree by orders of magnitude — the 36% of issue #64's volume with a non-positive parent holds almost none of its mass, so the volume fraction alone says most of the domain is undefined while the mass fraction alone says the state is nearly clean. Both are true of the same field.
- ![][badge-🐛bugfix] Correct the claim that the model keeps `ρq_tot` non-negative. `tracer_nonnegativity_method` is off by default and transport can take a cell negative on its own; the run in issue #64 had `ρq_tot ≤ 0` over 36% of its volume within the first hour. The non-positive-parent warning for the water family said this was unexpected, `energy_source_fraction` rested the water donor share on it, and the tagged-water and tracer-configuration pages both repeated it. All four now say what is true: where the parent is not positive the shares are undefined and those cells' tags mean nothing, and `nonpositive_fraction` in the closure table is what reports how much of the domain is in that state.
- [#63](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/63) ![][badge-✨feature/enhancement] The docs gain the tag-closure memo and the experiment plan that follows from it. The memo records what enforces the sum of the water, energy and energy source tags against their parent, what floating-point closure would cost per family, and why a monitored residual with a contract-style tolerance is the recommendation. The plan lays out the Levante experiments, water first, then energy, then the source tags, as instructions for an agent that prepares them; the runs themselves are started by the owner only.
- [#62](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/62) ![][badge-✨feature/enhancement] The parent-budget ledger logs a failed claim at warn level. After the summary, `write_parent_budget_report` lists every claim of the last accepted step whose status is `fail`, so a failed identity is visible in the run's log and not only in the certificate; the return code is unchanged. The docs gain the closure memo, which records why the calibrated `κ` model stays, and the closure plan, which lays out the `ε_state` term, the `Float32`, GPU and multi-rank rows and the sweep's ratio assertion as work items with their edit points.
- [#60](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/60) ![][badge-✨feature/enhancement] The parent-budget ledger publishes its claim certificate and calibrates its tolerance. A successful run with the ledger on writes `parent_budget_report.yaml`, a versioned certificate with the configuration, backend, rank count, float types, timestepper and adapter versions, the tolerances and their source, the restart segmentation, and for every control volume and quantity the parent verdict with its cumulative totals and the attribution and transfer verdicts of the last step, and logs a concise summary. `κ` is calibrated by the contract's protocol on a named moist slab column and committed as `src/parent_budget/kappa_calibration.yaml`; a run takes its row's tolerance for its backend, float type and rank count unless it passes `parent_budget_tolerances`, and a run with no row stays blocked by name. The tests re-measure the serial row, gate summary mode on constant per-step allocation, a bounded adapter and an explicit overhead beside a run without the ledger, and cross-check the ledger against `check_conservation` on a small moist sphere, where every identity holds under the column-calibrated `κ` and the check's residual is the turbulent flux and precipitation its callback omits.
- [#59](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/59) ![][badge-✨feature/enhancement] The parent-budget ledger survives a restart and admits declared callbacks. A checkpoint written with the ledger on carries the ledger's endpoint of the state it holds, as HDF5 attributes beside the model hash, and a restarted run measures the restored state and compares it with that endpoint exactly before its first transaction opens: a difference is refused as a change nobody accounted for, and a checkpoint written without a ledger restarts the record as unverified. The record after a restart is a new segment. A custom callback is accepted beside the ledger only inside `Internals.ParentBudget.ReadOnlyCallback`; `audit` mode reads the state around every firing and refuses one that wrote it, `summary` mode trusts the declaration. A callback that supplies its own accounting is still refused.
- [#58](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/58) ![][badge-✨feature/enhancement] The parent-budget ledger records every transfer leg in `audit` mode, one per weighted stage, through the applied-update events: the atmosphere's side of the surface flux and both sides of precipitation as the bracket's own total, the two radiation crossings and the slab's turbulent, radiative and prescribed fluxes from the flux fields their tendencies read, with the bracket's total kept beside their sum as a check. Each leg carries the channel it is applied through, so one-moment fallout under explicit microphysics is one event on two channels. Zero-moment precipitation is declared two-sided with a slab, the slab's own solve defect is booked, and a column's slab integrals use the point's quadrature weight, which they had been missing by half. On a dry slab column the surface flux cancels in the coupled view and crosses in the atmosphere-only view; on a moist DYCOMS slab column every declared transfer passes or is reported and every identity holds in both views.
- [#57](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/57) ![][badge-✨feature/enhancement] The parent-budget ledger attributes every collected channel to its processes. One applied-update event, `open_applied_update!` and `close_applied_update!`, replaces the tag brackets around every process that writes a parent field and feeds the tag families, the process records and the ledger at once; the implicit path calls the ledger half directly. In `audit` mode the adapter reads what each bracket applied at every weighted stage, pointwise and split into positive and negative parts, and books one leg per row and stage with the accepted weight; a row the coverage registry proves zero is booked from the registry in every mode. The new config key `parent_budget_attribution` (`net` by default, `gross`) keeps the parts beside the net amount the identities use. A transfer event a channel applies blocks that channel's attribution until its legs are recorded. Every measured component now carries the arithmetic magnitude of the sum behind it, which is what the tolerance's arithmetic term reads, since an envelope of a conservative operator cancels to a net far below its own rounding noise; and a run without the upwind correction hook records the solve defect as unknown instead of failing at the commit. The idealized flux-form radiation modes now declare the two radiation crossings, and TRMM_LBA a prescribed-heating row.
- [#56](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/56) ![][badge-✨feature/enhancement] The parent-budget ledger's parent identity can now pass. The adapter captures the implicit envelope from the stored effective implicit tendencies, meters the `lim!`, `dss!` and `constrain_state!` hooks to book the final accepted-state maps by their position in the step, and in `audit` mode books the solve defect, the post-implicit correction and the hooks the stepper folds into the implicit tendency as per-stage rows with their accepted weights, keeping every other firing as a stage observation. `parent_budget_tolerances` passes explicit tolerances until the calibrated table exists. Channel rosters carry per-row dispositions. The simulation-driven ledger tests move to a new `parent_budget` test group.
- [#55](https://github.com/johannespletzer/ClimaAtmosResiDyn.jl/pull/55) ![][badge-✨feature/enhancement] Add the parent-budget timestepper adapter and the executable coverage registry. The new config key `parent_budget_mode` (`off` by default, `summary`, `audit`) runs the ledger as the first callback of every accepted step: one packed global collective per step for the endpoints and the two explicit channels' envelopes, the trajectory bitwise unchanged, ClimaTimeSteppers pinned by a recorded version and a hook-order trace test. The schema now comes from `coverage_registry.jl`, which the coverage page is tested against cell by cell. The implicit envelope, the final maps, the rosters and the transfer legs are still unrecorded, so every parent claim is blocked naming them, which is the fail-closed core at work.
...
- ![][badge-🐛bugfix] Split the `dynamics` CI group in three. Its ten files took 75 to 92 minutes on GitHub's runners against the job's 90-minute limit, and a job cancelled at the limit reports as failed while never running the files that had not started, so `ci 1.10 - dynamics` was cancelled on three pull requests in a row. Almost all of the time is compilation of freshly built models, and two files carry most of it: on one CPU with Julia 1.11 the tracer/mass consistency file took 24 minutes and the EDMFX horizontal diffusion file 17, of 62 in all. The tracer transport and water-limiter files now run as `dynamics_tracers`, the two EDMFX diffusion files as `dynamics_edmfx`, and the other five stay in `dynamics`, each group under half the limit. Both workflow matrices and the test-group table list the new groups.
...
- ![][badge-✨feature/enhancement] Add energy source tags, under the new config key `energy_source_tags`, adding one prognostic field `ρe_src_<name>` per tag. A source tag holds moist energy attributed to where it came from, which is the energy counterpart of the water tags and a different quantity from `ρe_tag_<name>`, a signed process tag. Both families take the same `source` labels and only the rule applied to them differs. Off by default. **Experimental**: the amount-of-energy and provenance reading holds only where `ρe_tot` is positive, so the donor share is defined, and while the tag itself is non-negative.
  - The rule. A bracketed increment splits as `Δ = Δ⁺ - Δ⁻`, production mask-weighted (`M_k·Δ⁺`) and loss donor-proportional (`φ_k·Δ⁻`, `φ_k = ρe_src_k/ρe_tot`), reaching every tag whatever it lists. That asymmetry is what makes a tag an amount rather than a running total. `energy_source_fraction` is the guarded share: `water_tag_fraction` can rely on `ρq_tot > 0` because the model enforces it, while `ρe_tot` has no physical zero, so the fallback returns zero where the parent is not positive.
  - Not guaranteed non-negative, and three separate things can take a tag below zero: donor loss bounds the depletion *rate* rather than the amount removed over a finite step, so one step takes roughly `dt·φ_k·Δ⁻`; the share is undefined wherever `ρe_tot ≤ 0`, where no loss is applied at all; and the tags ride unlimited transport with no partition repair. These are limits of the discrete implementation, and a negative value invalidates the amount reading for that tag. `e_src_res` will not show it — that residual covers the pure region tags only — so inspect `e_src_<name>` directly, and use the initialization warning and the closure check's `nonpositive_fraction` for the parent.
  - Only the explicit tendency path is bracketed, as for the process records, so `precipitation` does not reach a source tag even though it is attributed to the `ρe_tag_*` family on the implicit path. `energy_source_tags` warns at configuration time about labels that cannot fire, and `ρe_tot` is checked for non-positive cells once at initialization whether or not the optional closure check is enabled.
  - New diagnostics `e_src_<name>` and the closure residual `e_src_res`, the parent minus the sum of the pure region tags, divided by density. It is an energy per unit mass and **not** a ratio: the `relative` and `gross_relative` columns of `energy_source_tag_closure.csv` are the ratios, normalized by `∫|ρe_tot|`. New `energy_source_closure_check` key. Tags are excluded from both tracer limiters through `is_tagged_tracer_name`.
  - Moist total energy has no physical zero, unlike total water, so tag values and their shares depend on the chosen thermodynamic and gravitational energy reference. That dependence is the open question these tags exist to measure, and it is stated on the new `docs/src/energy_source_tags.md` page rather than left implicit. Testing boundary: configured solves cover bracketed production, transport and restart; the loss algebra is covered against a synthetic positive parent; donor-proportional loss through a real solve is **not** validated, because no configured run yet keeps `ρe_tot` positive.
- ![][badge-🐛bugfix] Stop the tagging integration tests reporting success over a crashed solve. `solve_atmos!` catches a failure and returns `:simulation_crashed` rather than throwing, and every one of these tests discarded the return value, so the assertions ran against a prematurely terminated state. The tagged-water solve had been dying in `exner_given_pressure` on a negative pressure while its file reported 87 of 87 assertions passing. All nine solve sites across the four files now check the return code. The crash itself was a geometry bug: `base_config` put a DYCOMS_RF02 boundary layer on the default 30 km column, extrapolating a 1.5 km profile to where pressure reaches zero, and left the `z_center = 600` region partition in the bottom 2% of the domain. It now uses the shipped DYCOMS geometry. The four solve sites in the `restarts` group are covered too, using that group's `local_success` accumulator rather than a bare `@test`. `test/runtests.jl` also now rejects an unknown `TEST_GROUP` instead of running nothing and exiting zero, which is how splitting the `tagging` group left `downgrade.yml` asking for a group that no longer existed: both downgrade jobs could go green without running a single tagging test.
- ![][badge-🐛bugfix] Split the `tagging` CI group into `tagging_energy`, `tagging_water`, `tagging_source` and `tagging_record`, one file each. A tag name is a type parameter, so each tag set recompiles the whole tendency and solve pipeline and the files share no compilation; combined they overran the 90-minute job timeout on Julia 1.11, where `tagged_tracers_integration.jl` alone took 36m57s at 99% compilation. The failure mode is worse than a red group: the job was cancelled before the energy-source and process-record files started, so a timeout silently drops coverage of whatever had not yet run.
...
- ![][badge-✨feature/enhancement] Add process-change records, under two new config keys `energy_process_record` and `water_process_record`. Each names processes the same way a tag's `source` does, and accumulates the signed increment that process applies to `ρe_tot` or `ρq_tot`, output as `e_prc_<process>` and `q_prc_<process>`. This is the diagnostic a tag is not: a tag says what share of what is present came from somewhere, a record says what one process did, so gains and losses cancel rather than being split and attributed. Off by default and independent of the tags — either key works with no tags configured.
  ...
  - Records are written as instantaneous samples, not time averages. `default_diagnostics` had been putting `e_prc_*` and `q_prc_*` through `frequency_averages` alongside the tag fields. A record is a running total from the start of the simulation, so a window mean reports only where that total happened to sit mid-window, a variable-rate history cannot be recovered from a sequence of such means, and the difference of two averages is not the change over the interval between them. They now go through a new `frequency_snapshots`, which uses the same period ladder with no reduction in time, so the endpoint values a budget needs are the values written. Anything reading `e_prc_*` or `q_prc_*` from default output gets a different quantity than before.
  - Process labels that cannot record anything now warn at configuration time, as the energy source tags already do. `precipitation` has no explicit bracket at all, and `microphysics` is bracketed only when microphysics is stepped explicitly, which is not the default. A record that stays zero for one of these reasons reads exactly like a process that genuinely did nothing, and nothing downstream can tell them apart.
  - The `energy_process_record` help text said "Not a prognostic field". Records became prognostic when the accumulator was fixed, and the help now says so, along with the reason a prognostic field is still not a tracer.
  - New `test/process_record_integration.jl` covers a records-only configured solve: the fields reach the state and start at zero, nothing in the automatic transport machinery picks them up, the timestepper integrates them, and they survive a checkpoint with their values intact.
