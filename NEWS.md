...
main
----
- ![][badge-✨feature/enhancement] Add the parent-budget ledger's journal core, in the new `src/parent_budget/` subtree. It is the accounting layer that will reconcile mass, total water and total energy against the accepted discrete update, and this step adds the schema, the authoritative integrals and the per-step transaction. Nothing is wired into the timestepper yet, no tendency is instrumented, and no simulation output changes. A leg records one signed extensive amount for one reservoir, and every `(ΔM, ΔW, ΔE)` component carries a status: measured, proven invariant zero, not applicable, or unknown. Unknown contributes nothing to a sum and blocks the closure claim for its quantity, so a component nobody measured cannot read as zero, and a projection reports which endpoints and legs blocked it rather than only that it is blocked. A projection also reports whether the quantity is applicable in that view at all, so water in a dry model does not read as a budget that closed at zero. Legs are recorded once, keyed by event, leg, step, stage and occurrence: the stage index is what lets one correction fire at every ARS343 stage without its firings colliding as duplicates. Control-volume totals are projections of those legs, so one surface exchange is a boundary crossing for the atmosphere alone and internal to the coupled system without being recorded twice, and an expected cancellation is measured through `transfer_mismatch` rather than imposed. The residual is defined only as endpoint change minus recorded, and three cumulative numbers are kept per quantity and control volume, because a signed sum cancels the very failure the ledger exists to expose. Consecutive transactions must meet at the same endpoint, which is checked rather than assumed. The endpoint integrals follow the contract: total water is `∫ρq_tot` alone with the categories partitioning it, `D = M - W` is the dry-air mass, and the slab surface owns mass as well as water and energy.
