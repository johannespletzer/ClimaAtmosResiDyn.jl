...
main
----
- ![][badge-✨feature/enhancement] Add the parent-budget ledger's journal core, in the new `src/parent_budget/` subtree. It is the accounting layer that will reconcile mass, total water and total energy against the accepted discrete update, and this step adds the schema, the authoritative integrals and the per-step transaction. Nothing is wired into the timestepper yet, no tendency is instrumented, and no simulation output changes. A leg records one signed extensive amount for one reservoir, and every `(ΔM, ΔW, ΔE)` component carries a status: measured, proven invariant zero, not applicable, or unknown. Unknown contributes nothing to a sum and blocks the closure claim for its quantity, so a component nobody measured cannot read as zero. Legs are recorded once and refused on a repeated `(event, leg, step)`, and control-volume totals are projections of those legs, so one surface exchange is a boundary crossing for the atmosphere alone and internal to the coupled system without being recorded twice. An expected cancellation is measured through `transfer_mismatch` and never imposed. The residual is defined only as `endpoint change - recorded`, and there is no function that can create a balancing entry. The endpoint integrals follow the contract: `∫ρ` excludes precipitating water, total water is `ρq_tot + ρq_rai + ρq_sno` and never `ρq_lcl` or `ρq_icl`, the dry-air invariant is `∫(ρ - ρq_tot)`, and the slab surface owns energy and water but no mass.

- ![][badge-✨feature/enhancement] Add the parent-budget ledger plan, budget contract and coverage matrices, as three new Developer Guide pages under `docs/src/parent_budget/`. This is design only: no instrumentation, no code, and no change to any simulation. It freezes what an exact signed discrete mass, total-water and total-energy ledger will claim, and inventories every equation and state-mutation path in the model against that claim. Reviewing the code against the earlier draft plan changed several of its premises. `ρ` carries vapour and cloud condensate but not rain or snow, so the dry-air invariant is `∫(ρ - ρq_tot)` and not `M - W`, and `ρq_lcl` / `ρq_icl` must be left out of total water because they are already inside `ρq_tot`. `remaining_tendency!` writes two explicit channels, and horizontal tracer advection and tracer hyperdiffusion live only in the limited one, so an adapter reading `Yₜ` alone would lose them. The default `NewtonsMethod(max_iters = 1)` does not converge the implicit stage, which makes the algebraic solve defect a leading-order term rather than a small correction. Fixed-step IMEX never rejects a step, so rejection handling becomes an assertion instead of an implementation. The only prognostic surface reservoir is the slab, which owns energy and water but no mass. The pages also record four reasons the existing `check_conservation` is a cross-check rather than authoritative accounting, among them that its boundary term is `dt` times one sampled radiative flux and that it omits turbulent surface fluxes.
- ![][badge-🐛bugfix] Reattach the `tag_closure_callback!` docstring to its function, which fixes precompilation on Julia 1.10. `nonpositive_parent_note` and its own docstring were inserted between the two, so the first docstring was left pointing at the second one. A docstring cannot document a string literal, and Julia refuses the whole module with `cannot document the following expression`. The symptom was a `Failed to precompile ClimaAtmos` in the invalidations job. Nothing but the position of the docstring changes, and `tag_closure_callback!` is documented again, which it had silently stopped being.
- ![][badge-✨feature/enhancement] Add the parent-budget ledger plan, budget contract and coverage matrices, as three new Developer Guide pages under `docs/src/parent_budget/`. This is design only: no instrumentation, no code, and no change to any simulation. It freezes what an exact signed discrete mass, total-water and total-energy ledger will claim, and inventories every equation and state-mutation path in the model against that claim. Total water is `∫ρq_tot` alone: the thermodynamic state builds `q_liq = q_lcl + q_rai` and `q_ice = q_icl + q_sno`, so rain and snow are inside `q_tot` and the categories partition it rather than adding to it. `ρ` tracks `ρq_tot` on every path that changes it, so `D = M - W` is the dry-air mass. One-moment precipitation formation is internal to mass, water and energy alike, and the atmosphere loses water through sedimentation in `vertical_advection_of_water_tendency!`, whose lower boundary is open; 0-moment removal is a direct sink instead. `Y.sfc.water` owns mass as well as water, which is why the existing `check_conservation` adds its change to the change in `∫ρ`. `remaining_tendency!` writes two explicit channels, and horizontal tracer advection and tracer hyperdiffusion live only in the limited one, so an adapter reading `Yₜ` alone would lose them. The default `NewtonsMethod(max_iters = 1)` does not converge the implicit stage, which makes the algebraic solve defect a leading-order term rather than a small correction. Fixed-step IMEX never rejects a step, so rejection handling becomes an assertion instead of an implementation. The pages also record four reasons the existing `check_conservation` is a cross-check rather than authoritative accounting, among them that its boundary term is `dt` times one sampled radiative flux and that it omits turbulent surface fluxes.
