...
- ![][badge-✨feature/enhancement] Add energy source tags, under the new config key `energy_source_tags`, adding one prognostic field `ρe_src_<name>` per tag. A source tag holds moist energy that is present now, traced back to where it came from; it is the energy counterpart of the water tags and a different quantity from `ρe_tag_<name>`, which holds a signed process-change record. Both families take the same `source` labels, and only the rule applied to them differs. Off by default.
  - This first change carries state, masks, transport, restarts and diagnostics only. The never-below-zero attribution rule — masked production and donor-proportional loss, `Δ(ρe_src_k) = M_k·Δ⁺ − φ_k·Δ⁻` — is not implemented yet, so a region tag is initialized to its masked share and then only transported, and a tag carrying a `source` stays at zero. Configure region tags only for now; the docs say so.
...
- ![][badge-✨feature/enhancement] Add process-change records, under two new config keys `energy_process_record` and `water_process_record`. Each names processes the same way a tag's `source` does, and accumulates the signed increment that process applies to `ρe_tot` or `ρq_tot`, output as `e_prc_<process>` and `q_prc_<process>`. This is the diagnostic a tag is not: a tag says what share of what is present came from somewhere, a record says what one process did, so gains and losses cancel rather than being split and attributed. Off by default and independent of the tags — either key works with no tags configured.
  - Records are cache-resident, not prognostic. Nothing transports them, no limiter touches them, and they add no Jacobian block. The increment they need is the one the existing `snapshot_tags!` / `attribute_tags!` bracket already differences, so a record costs one field per process and one broadcast per process per step.
  - Only the explicit tendency path is recorded, because the implicit path is evaluated with `ForwardDiff.Dual` numbers when an automatic-differentiation Jacobian is used and the cache holds plain floats. So `microphysics` on the water side and `precipitation` on the energy side stay zero when stepped implicitly, which is the default for microphysics. Transport, phase changes, gravity-wave drag and numerical corrections have no bracket and are absent entirely. A run's records therefore do not sum to the change in the parent variable, and are not meant to: they are a per-process history over the bracketed processes, not a closed budget. The docs say so.
  - Cumulative within a simulation segment and reset on restart, like `q_tag_fix_<name>`, so a budget over an interval is the difference of two outputs and a time average is not meaningful.
  - New user-facing page `docs/src/process_record.md`.

- ![][badge-✨feature/enhancement] Say what an energy tag configured with `source` actually holds. It starts at zero and accumulates the signed increment one process adds, so it is a process-change record and goes negative under net cooling, not a share of the energy present — which is what "source" names in the water family, where production is mask-weighted and loss donor-proportional. The docs and comments now call it a process tag; the `source` key, the `ρe_tag_*` fields and the rule are unchanged. Also states two limits the water page carried and the energy page did not: closure is internally consistent accounting rather than a counterfactual sensitivity, and moist total energy has no physical zero, so region-tag values and any residual normalized by `max|ρe_tot|` move with the chosen energy reference.
...
- ![][badge-✨feature/enhancement] Add `chemistry_model: "stratospheric_passive_tracers"`, a family of inert tracers for measuring stratospheric residence times. Each is produced at a constant mass-fraction rate inside one small source box above the tropopause — 10° of latitude and 2 km deep by default — and relaxed to zero at and below the tropopause, so once its burden stops drifting its lifetime is `burden / source`. The boxes sample the domain rather than tiling it: a tracer emitted over a deep layer or a wide latitude range would report a lifetime averaged over conditions that differ by years, so gaps between boxes are allowed, while overlaps — which would make a point feed two tracers — are refused. The global burden, source rate and loss rate of every tracer are appended to `stratospheric_tracer_budget.csv` every `dt_tracer_budget`, and `post_processing/tracer_lifetimes.jl` turns that table into lifetimes and flags the tracers that are not yet in equilibrium. Replaces the 18-gas `examples/passive_stratospheric_tracers.jl` prototype, whose tracers each mixed eight source altitudes and so had no single source region to attribute a lifetime to.
