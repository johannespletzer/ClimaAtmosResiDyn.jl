...
main
----
- ![][badge-✨feature/enhancement] The energy source tags' closure check reports more about its residual, all read-only. Every row of `energy_source_tag_closure.csv` carries the offset's headroom, `headroom_min` (the smallest `e_tot + c`, J/kg) and `headroom_min_z`, after the spin-up columns and before `void`. With `audit: true` the audit table says where the residual sits (`residual_max`, `residual_peak_level` and their heights) and whether the source tags leave the partition's bounds (`overlay_negative_mass_fraction`, `overlay_excess`). With `energy_source_tag_ledger_per_tag: true` a new state ledger, `e_src_led_src_res`, records what the source brackets did to the residual, the loss rule's flush; the audit gives its gross, the flush rate and the level the residual would settle at, and the closure row the gross source throughput and `gross_over_throughput`. A second warning level, `throughput_tolerance`, off by default, warns on that ratio, whose scale is set by the sources rather than by the energy reference. The closure checks' warning now says it is a warning level and not an acceptance threshold. The model's fields are unchanged.
