...
main
----
- ![][badge-✨feature/enhancement] Give the energy source tags their attribution rule, so they now hold what their name says. A bracketed increment is split as `Δ = Δ⁺ - Δ⁻`, production is mask-weighted (`M_k · Δ⁺`) and loss is donor-proportional (`φ_k · Δ⁻` with `φ_k = ρe_src_k / ρe_tot`), reaching every tag whatever processes it lists. That asymmetry is what makes a tag an amount of energy present rather than a running total, and what keeps a loss it does not own from driving it negative. It is the rule the water tags already use, and the opposite of the `ρe_tag_*` family. New `energy_source_closure_check` key, written to `energy_source_tag_closure.csv`. Note the testing boundary: configured solves cover bracketed production, transport and restart, and the loss algebra is covered against a synthetic positive parent, but donor-proportional loss through a real solve is not validated — no configured run yet keeps `ρe_tot` positive, so the share is undefined and the loss term does not run. See `docs/src/energy_source_tags.md`.
  ...
  - The tags are **not** guaranteed non-negative, and the docs say so plainly rather than promising it of the attribution step. Donor-proportional loss bounds the *rate* a tag is depleted at, not the amount removed over a finite step: attribution yields a tendency, so one step takes roughly `dt * φ_k * Δ⁻`, and parent tendencies that cancel leave `ρe_tot` unchanged while a tag receiving only the loss half crosses zero. Clamping `φ_k` to `[0, 1]` cannot prevent that, because the clamp acts on the share and the step length sets the amount. Two further routes: where `ρe_tot ≤ 0` the share is undefined and no donor loss is applied at all, and the tags ride unlimited explicit transport with no partition repair. A negative tag is information about the timestep, the energy reference or the transport, and is watched through `e_src_res`.
