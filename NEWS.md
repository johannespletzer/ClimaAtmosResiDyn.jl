...
main
----
- ![][badge-✨feature/enhancement] Add `water_tag_leak_correction`, `false` by default: under 1-moment microphysics and `edmfx_sgs_diffusive_flux: true`, each water tag takes back the EDMF vertical diffusion of its share of the rain and snow, the share the sedimentation takes it by. The parent diffuses the water without rain and snow at `K_h` and the tags their whole value, so without it the partition gains the diffusion of `q_rai + q_sno` (`q_tag_leak_vdiff`), and under `water_tag_transport: increment` the follower absorbs that leak and spreads it by the shares of the cells its flux leaves. With it the partition's diffusion is the parent's, and the leak is charged to the tags whose water leaked. Each updraft copy takes its tag's correction, as it takes its tag's diffusion. New ledgers: `q_tag_led_leaknet` and, with copies, `q_tag_led_upleaknet`, and under `water_tag_ledger_per_tag: true` each tag's `q_tag_led_leak_<name>` and `q_tag_led_upleak_<name>`, with their per-step grosses. The key is refused without water tags, without 1M microphysics, without the EDMF diffusive flux, and with `vert_diff`, whose leak it does not correct. A restart that changes it is refused. The model's fields are unchanged, bit for bit. The new test group `tagging_water_leak` checks it on the D4-W EDMF column.
