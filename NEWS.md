...
main
----
- ![][badge-🔨maintenance] The energy source tags' exchange at the sub-grid mass flux allocates per cell on the oldest dependency versions the fork's compat allows, about 17 kB per call on the DYCOMS column, where on the versions it runs it allocates only the 8 bytes of the parent's environment helper. The environment's density comes from a thermodynamic call, and the broadcasts that reach through that chain stop inferring on the older packages. The kernel itself is allocation-free on both, measured on ClimaCore 0.16.0 and 1.0.0 in isolation, and writing the chain to scratch costs more on the current versions rather than less. The two allocation tests therefore take the strict bound on current versions and a loose one on the downgrade matrix. It is a performance limitation of the oldest supported stack, not of the versions the fork runs, and the results are identical either way.
