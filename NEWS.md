...
main
----
- ![][badge-✨feature/enhancement] `water_tag_precipitation: true` splits each of the `water_tracers` into three parts: `ρq_tag_<name>` then holds only the water that is neither rain nor snow, `ρq_rtag_<name>` the rain and `ρq_stag_<name>` the snow, each a share of its own part of the parent (stage 1 of the design note WP4b-D, a 1-moment column or grid without EDMF). Rain and snow fall in their own parts with their species' own Jacobian blocks, and take no diffusion, hyperdiffusion or sponge. The microphysics moves water between a tag's parts by its gross process flows, each with its donor's composition, and records the difference from the net-flow rule in `q_rtag_aud_<name>` and `q_stag_aud_<name>`. The limiters and constraints move each part with its compartment, and the repair runs per compartment. New diagnostics: `q_ntag_<name>`, `q_rtag_<name>`, `q_stag_<name>`, the compartments' residuals `q_ntag_res`, `q_rtag_res` and `q_stag_res`, and `pr_tag_<name>`, the tag's share of `pr`; `q_tag_<name>` is the tag's total. The key needs `microphysics_model: 1M` and is refused under EDMF and with `water_tag_updraft_copy: true`. The water tags' checkpoint format goes to version 2, which records the key, and a restart that changes it is refused. Tag names starting with `aud_` are refused. Without the key nothing changes, and with it the model's own fields are those without tags, bit for bit.
