...
# shellcheck disable=SC2054 # commas below are part of Slurm option values
...
levante_gpu_init() {
    ...
    # Every rank needs a distinct GPU, which levante_gpu_report_binding enforces
    # per rank. Levante allocates complete four-GPU nodes even for the 1- and
    # 2-rank variants, so unused reserved GPUs are expected; only a GPU deficit
    # is an error.
    ...
    if [[ -n "${SLURM_GPUS_ON_NODE:-}" ]] &&
       (( tasks_per_node > SLURM_GPUS_ON_NODE )); then
        echo "ERROR: ${tasks_per_node} ranks per node but only ${SLURM_GPUS_ON_NODE} GPUs on the node." >&2
        echo "  Each rank requires a distinct GPU." >&2
        exit 1
    fi
