...
- Format code before committing. **CI does not check formatting** — [.github/workflows/run-prek.yml](.github/workflows/run-prek.yml) sets `SKIP: julia-formatter`, so nothing catches drift for you and running the formatter yourself is the only thing keeping the tree clean. The `julia-formatter` hook is still defined in [.pre-commit-config.yaml](.pre-commit-config.yaml) and runs JuliaFormatter from the version-pinned [.dev/format/Project.toml](.dev/format/Project.toml) environment (currently `=2.10.1`). Run it with either `prek run julia-formatter --all-files` or the pinned env directly: `julia --startup-file=no --project=.dev/format -e 'using Pkg; Pkg.instantiate(io=devnull); using JuliaFormatter; format(ARGS)' .`. Avoid `julia -e 'using JuliaFormatter; format(\".\")'` from your global environment — `Pkg.add("JuliaFormatter")` installs v2 by default and a mismatched version produces a different diff.
