...
## Codebase map
  ...
  - `src/parameterized_tendencies/`: parameterization implementations.
      ...
      + `tagged_tracers/`: tagged prognostic energy (`tagged_tracers.jl`, `ρe_tag_*`, config key `energy_tracers`) and water (`tagged_water.jl`, `ρq_tag_*`, config key `water_tracers`) tracers — smooth region masks, state builders, and the snapshot/attribute brackets consumed by `remaining_tendency.jl` and `implicit/implicit_tendency.jl`. `process_record.jl` holds the process-change records (`prc_*` cache fields, config keys `energy_process_record` and `water_process_record`), which reuse the same brackets but are cache-resident rather than prognostic. Their config parsing lives in `src/config/tracer_config.jl`.
