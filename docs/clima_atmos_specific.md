...
## Codebase map
  ...
  - `src/parent_budget/`: the parent-budget ledger, the accounting layer that reconciles mass, total water and total energy against the accepted discrete update. `journal.jl` holds the leg schema, reservoirs and control volumes; `integrals.jl` holds the authoritative endpoint integrals; `transaction.jl` holds the per-step transaction, the projections and the residual. It is a diagnostic and never writes to the state. Its contract and coverage matrices are in [contract.md](src/parent_budget/contract.md) and [coverage.md](src/parent_budget/coverage.md).
