...
## Codebase map
  ...
  - `src/parent_budget/`: the parent-budget ledger, the internal accounting layer that reconciles mass, total water and total energy against the accepted discrete update. `integrals.jl` defines the accounting precision and the three authoritative integrals, all local so nothing there communicates; `reduction.jl` holds the fixed packet layout and the single global collective a step spends; `journal.jl` holds the leg schema, per-component evidence, reservoirs, control volumes and collection levels; `transaction.jl` holds the per-step transaction and the three separate reconciliations, parent, attribution and transfer. It is a diagnostic and never writes to the state, and nothing in it is exported. **Nothing is wired into a simulation yet**: there is no configuration key, no tendency is instrumented and no output changes, so the ledger has to be driven explicitly from a test. The timestepper adapter that will capture accepted channel envelopes belongs to a later stack step and will live in the same subtree; it is the only place allowed to know about `ClimaTimeSteppers` stage weights and hook order. The normative documents are [contract.md](src/parent_budget/contract.md), [architecture.md](src/parent_budget/architecture.md), [coverage.md](src/parent_budget/coverage.md) and [plan.md](src/parent_budget/plan.md).
