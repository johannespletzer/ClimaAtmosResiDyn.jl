...
final maps. Each is the complete update one integrator channel applied, taken
from the applied increment and never from endpoint subtraction.
...
| `env.final_maps`       | accepted-state maps in `lim!`, `dss!`, `constrain_state!` | always                                 | finalization  | atmosphere | `ρ`, `ρq_tot`, `ρe_tot`       | measured · measured · measured | raw before/after difference on the accepted state                    | final map | none  | ordered before/after pair around each map                             | `final_map_tests.jl` | 7    |
