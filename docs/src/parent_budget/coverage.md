...
|:--|:--|:--|:--|:--|:--|
| `horizontal_tracer_advection_tendency!` | `ρq_tot`, `ρq_rai`, `ρq_sno`, tracers | `zero` | `zero` | `zero` | Conservative horizontal divergence; global zero is an invariant of the operator and is **tested**, not assumed |
...
| `microphysics_tendency!` | `ρq_tot`, `ρq_rai`, `ρq_sno`, `ρ`, `ρe_tot` | `measured` | `zero` | `measured` | only when stepped explicitly. Formation is **internal to W**, and a **real mass sink** because `ρ` excludes precipitation |
...
| `surface_precipitation_tendency!` | `Y.sfc.water`, `Y.sfc.T` | `n/a` | `measured` (surface) | `measured` (surface) | the surface leg of fallout; paired with the atmospheric microphysics sink |
...
| Path | Fields | ΔM | ΔW | ΔE | Classification |
|:--|:--|:--|:--|:--|:--|
| `implicit_vertical_advection_tendency!` | `ρ`, `ρe_tot`, tracers, `u₃` | `zero` | `zero` | `zero` | conservative transport, tested |
| `microphysics_tendency!` | as above | `measured` | `zero` | `measured` | the **default** path, since `implicit_microphysics` defaults on |
| `surface_precipitation_tendency!` | `Y.sfc.*` | `n/a` | `measured` (surface) | `measured` (surface) | the implicit counterpart of the fallout leg |
