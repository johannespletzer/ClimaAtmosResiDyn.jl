...
| `energy_source_tag_updraft_copy` | `false`        | leave off. `true` is the audit: a copy of each tag in the updraft, moved by the model's own tracer machinery, at about twice the build time           |
