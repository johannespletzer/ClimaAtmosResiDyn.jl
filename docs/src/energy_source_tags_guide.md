...
| `energy_source_tag_increment_allow_explicit_1m` | `false`        | leave off. `enthalpy_increment` with 1M stepped explicitly is refused, because the tags lag the parent's sedimentation there. `true` runs it anyway, for development |
